...
bool ModeHeart::init(bool ignore_checks)
{
    ...
    copter.heart_nav->init();

#if HAL_MOUNT_ENABLED
    AP_Mount *s = AP_Mount::get_singleton();

    // Check if the CIRCLE_OPTIONS parameter have roi_at_center
    if (copter.heart_nav->roi_at_center()) {
        const Vector3p &pos { copter.heart_nav->get_center() };
        Location heart_center;
        if (!AP::ahrs().get_location_from_origin_offset(heart_center, pos * 0.01)) {
            return false;
        }
        // point at the ground:
        heart_center.set_alt_cm(0, Location::AltFrame::ABOVE_TERRAIN);
        s->set_roi_target(heart_center);
    }
#endif

    // set auto yaw circle mode
    auto_yaw.set_mode(AutoYaw::Mode::HEART);
...
void ModeHeart::run()
{
    ...
    copter.heart_nav->check_param_change();

    // pilot changes to circle rate and radius
    // skip if in radio failsafe
    if (!copter.failsafe.radio && copter.heart_nav->pilot_control_enabled()) {
        // update the circle controller's radius target based on pilot pitch stick inputs
        const float radius_current = copter.heart_nav->get_radius();           // circle controller's radius target, which begins as the circle_radius parameter
        ...
        if (!is_equal(radius_current, radius_new)) {
            copter.heart_nav->set_radius_cm(radius_new);
        }
        ...
        if (g.radio_tuning != TUNING_CIRCLE_RATE) {
            ...
            if (is_zero(roll_stick)) {
                ...
                const float rate = copter.heart_nav->get_rate();           // circle controller's rate target, which begins as the circle_rate parameter
                const float rate_current = copter.heart_nav->get_rate_current(); // current adjusted rate target, which is probably different from _rate
                ...
                speed_changing = true;
                copter.heart_nav->set_rate(rate_new);
            }
        }
    }
    ...
    copter.failsafe_terrain_set_status(copter.heart_nav->update(target_climb_rate));
...
{
    return copter.heart_nav->get_distance_to_target();
}

int32_t ModeHeart::wp_bearing() const
{
    return copter.heart_nav->get_bearing_to_target();
}
